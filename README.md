...
If you need to map the `VIRTUAL_PATH` to some specific path on the destination you can add a `VIRTUAL_PATH_DEST` variable. For example with `VIRTUAL_HOST=foo.example.com` and `VIRTUAL_PATH=/api/v2/service` and `VIRTUAL_PATH_DEST=/v2/service`, then requests to http://foo.example.com/api/v2/service will be routed to the container with path `/v2/service`.
